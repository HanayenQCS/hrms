...
class PayrollEntry(Document):
	...
	def get_salary_components(self, component_type):
		...
		if salary_slips:
			...
			frappe.errprint("COmponents breakdonw")
			frappe.errprint(salary_components)
			#[{'salary_component': 'Basic', 'amount': 1000.0, 'parentfield': 'earnings', 'salary_structure': 'Hanayen UAE - 2023', 'employee': 'HAN- 434'}, {'salary_component': 'Allowance', 'amount': 377.5, 'parentfield': 'earnings', 'salary_structure': 'Hanayen UAE - 2023', 'employee': 'HAN- 434'}, {'salary_component': 'Allowance', 'amount': 1200.0, 'parentfield': 'earnings', 'salary_structure': 'Hanayen UAE - 2023', 'employee': 'HAN- 829'}, {'salary_component': 'Allowance', 'amount': 1200.0, 'parentfield': 'earnings', 'salary_structure': 'Hanayen UAE - 2023', 'employee': 'HAN- 830'}]
	...
	def get_salary_component_total(
		...
		if salary_components:
			...
			for item in salary_components:
				...
				frappe.errprint("emp cost center")
				frappe.errprint(employee_cost_centers)
			...
			frappe.errprint("Components")
			frappe.errprint(account_details)
			
			return account_details

	def get_emp_salary_component_total(self, component_type=None):
		salary_components = self.get_salary_components(component_type)
		component_dict = {}
		account_details = {}
		for item in salary_components:
			key = (item.employee, item.salary_component)
			component_dict[key] = item.amount
		frappe.errprint("New comp")
		frappe.errprint(component_dict)
		account_details = self.get_account_qcs(component_dict=component_dict)
		return account_details
	...
	def get_account(self, component_dict=None):
		...
		#frappe.errprint("acc_dict")
		#frappe.errprint(account_dict)

		return account_dict

	def get_account_qcs(self, component_dict=None):
		account_dict = {}
		for key, amount in component_dict.items():
			account = self.get_salary_component_account(key[1])
			accouting_key = (key[0], account)
	...
	def make_accrual_jv_entry(self):
		...
		self.employee_based_payroll_payable_entries = {} #setting global variable

		# via get_salary employee based payoll field is being populated
		...
		earnings = self.get_emp_salary_component_total(component_type="earnings")
		...
		deductions = self.get_emp_salary_component_total(component_type="deductions")
